  ...
  it('aggregates bytes, calculates percentages, and sorts descending for owned repos', () => {
    const repos: ViewerRepoNode[] = [
      ...
      },
      {
        ...
        languages: {
          edges: [
            { size: 1000, node: { name: 'Rust', color: null } },
          ],
        },
      },
      {
...
describe('buildRepositoryActivities', () => {
  it('unifies contributions and LOC records into non-duplicated items', () => {
    const repoContributions = new Map([
      ['user/repo-a', { commits: 10, issues: 2, pullRequests: 1, reviews: 0, isPrivate: false }],
      ['user/repo-b', { commits: 5, issues: 0, pullRequests: 0, reviews: 1, isPrivate: true }],
    ]);

    const locResults = new Map([
      ['user/repo-a', { repository: 'user/repo-a', is_private: false, is_fork: false, additions: 200, deletions: 50, commit_count: 8 }],
      ['user/repo-c', { repository: 'user/repo-c', is_private: false, is_fork: true, additions: 50, deletions: 10, commit_count: 2 }],
    ]);

    const viewerRepos: ViewerRepoNode[] = [
      { nameWithOwner: 'user/repo-a', name: 'repo-a', isPrivate: false, isFork: false, stargazerCount: 0, forkCount: 0, owner: { login: 'user' } },
      { nameWithOwner: 'user/repo-b', name: 'repo-b', isPrivate: true, isFork: false, stargazerCount: 0, forkCount: 0, owner: { login: 'user' } },
      { nameWithOwner: 'user/repo-c', name: 'repo-c', isPrivate: false, isFork: true, stargazerCount: 0, forkCount: 0, owner: { login: 'user' } },
    ];

    const result = buildRepositoryActivities({ repoContributions, locResults, viewerRepos });
  ...
  it('correctly calculates negative net lines when deletions exceed additions', () => {
    const locResults = new Map([
      ['user/refactor-repo', { repository: 'user/refactor-repo', is_private: false, is_fork: false, additions: 30, deletions: 120, commit_count: 3 }],
    ]);
