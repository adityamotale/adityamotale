...
function parseCliArgs(): {
  ...
  if (lastWeek) {
    return { dateRange: getSundayToSaturdayWindow(), isIncremental: true, dataDir };
  }
  ...
  if (fromArg || toArg) {
    const from = fromArg ? (fromArg.includes('T') ? fromArg : `${fromArg}T00:00:00Z`) : '2021-01-01T00:00:00Z';
    const to = toArg ? (toArg.includes('T') ? toArg : `${toArg}T23:59:59Z`) : new Date().toISOString();
    return { dateRange: { from, to }, isIncremental: !forceFull && cacheExists, dataDir };
  }
...
function printReport(stats: GitHubStatsOutput, mode: string): void {
  ...
  console.log(`Period:       ${stats.metadata.period.from} → ${stats.metadata.period.to}`);
  console.log(`Generated:    ${stats.metadata.generated_at}`);
  console.log('------------------------------------------------------');

  console.log('📊 1. LINES OF CODE (ALL REPOSITORIES)');
  console.log(`  - Total Additions: +${stats.lines_of_code.total_additions.toLocaleString()} lines`);
  console.log(`  - Total Deletions: -${stats.lines_of_code.total_deletions.toLocaleString()} lines`);
  console.log(`  - Net Code Change:  ${stats.lines_of_code.net_lines.toLocaleString()} lines`);

  console.log('\n💻 2. TOP 5 LANGUAGES');
  for (const lang of stats.languages.slice(0, 5)) {
    const bar = '█'.repeat(Math.round(lang.percentage / 5));
    console.log(`  - ${lang.name.padEnd(14)} ${lang.percentage.toFixed(2).padStart(6)}% [${bar}] (${(lang.bytes / 1024).toFixed(1)} KB)`);
  }

  console.log('\n📦 3. REPOSITORIES OWNED');
  console.log(`  - Total Owned:     ${stats.repositories.total_owned} (Public: ${stats.repositories.public}, Private: ${stats.repositories.private}, Forks: ${stats.repositories.forks})`);
  ...
  console.log(`  - Grand Total:     ${stats.contributions.total.toLocaleString()}`);
  console.log(`  - Public / Private:${stats.contributions.public.toLocaleString()} public | ${stats.contributions.private.toLocaleString()} private (${((stats.contributions.private / (stats.contributions.total || 1)) * 100).toFixed(1)}%)`);
  console.log(`  - Breakdown:       ${stats.contributions.commits.toLocaleString()} commits, ${stats.contributions.pull_requests.toLocaleString()} PRs, ${stats.contributions.issues.toLocaleString()} issues, ${stats.contributions.reviews.toLocaleString()} reviews`);

  console.log('\n🚀 5. TOP REPOSITORIES');
  console.log(`  - Distinct Repos:  ${stats.repositories_contributed_to.length} repositories`);
  for (const repo of stats.repositories_contributed_to.slice(0, 8)) {
    const tag = repo.is_private ? '[Private]' : repo.is_fork ? '[Fork]' : '[Public]';
    console.log(`    • ${repo.repository.padEnd(32)} ${tag.padEnd(10)} +${repo.additions.toLocaleString()}/-${repo.deletions.toLocaleString()} LOC | ${repo.total_contributions} contribs (${repo.commits}c, ${repo.pull_requests}pr)`);
  }

  console.log('\n📈 6. STREAKS & CONSISTENCY');
  console.log(`  - Longest Streak:  ${stats.streaks_and_consistency.longest_streak_days} days`);
  console.log(`  - Current Streak:  ${stats.streaks_and_consistency.current_streak_days} days`);
  console.log(`  - Active Days:     ${stats.streaks_and_consistency.active_days_count} / ${stats.streaks_and_consistency.total_days_in_period} days (${stats.streaks_and_consistency.activity_rate_percentage}%)`);
  if (stats.streaks_and_consistency.busiest_day.date) {
    console.log(`  - Busiest Day:     ${stats.streaks_and_consistency.busiest_day.date} (${stats.streaks_and_consistency.busiest_day.count} contributions)`);
  }

  console.log('\n🔄 7. PULL REQUESTS');
  console.log(`  - Total Opened:    ${stats.pull_requests.total}`);
  console.log(`  - Merged:          ${stats.pull_requests.merged} (${stats.pull_requests.merge_rate_percentage}% merge rate)`);
...
async function executeIncremental(
  ...
  console.log(`Executing incremental update for window: ${window.from} → ${window.to}`);
  ...
  const grandTotal = Object.values(daily).reduce((sum, count) => sum + count, 0);
  ...
  const candidateRepos = activeNames.map((name) => {
    ...
    return {
      nameWithOwner: name,
      isPrivate: meta?.isPrivate ?? windowData.repoContributions.get(name)?.isPrivate ?? false,
  ...
  const repoMap = new Map(existing.repositories_contributed_to.map((r) => [r.repository, { ...r }]));
  ...
  for (const [name, stat] of windowData.repoContributions.entries()) {
    ...
    const contribCount = commitCount + stat.issues + stat.pullRequests + stat.reviews;
    ...
    if (repoMap.has(name)) {
      ...
      repoMap.set(name, {
        ...
        is_fork: candidateRepos.find((r) => r.nameWithOwner === name)?.isFork ?? false,
        commits: commitCount,
        issues: stat.issues,
  ...
  return {
    ...
    contributions: {
      ...
      pull_requests: existing.contributions.pull_requests + windowData.pullRequests,
      ...
      public:
        ...
        (windowData.commits + windowData.pullRequests + windowData.issues + windowData.reviews),
    ...
    pull_requests: {
      ...
      merged: mergedPRs,
      open: windowPRs.open,
      closed_unmerged: existing.pull_requests.closed_unmerged + windowPRs.closed_unmerged,
      merge_rate_percentage: totalPRs > 0 ? parseFloat(((mergedPRs / totalPRs) * 100).toFixed(2)) : 0,
    },
...
async function executeFullBackfill(dateRange: DateRange, viewer: ViewerContext): Promise<GitHubStatsOutput> {
  console.log(`Executing full historical backfill: ${dateRange.from} → ${dateRange.to}`);
  ...
  for (let year = startYear; year <= endYear; year++) {
    const from = year === startYear ? dateRange.from : `${year}-01-01T00:00:00Z`;
    ...
    for (const [name, stat] of yearData.repoContributions.entries()) {
      const entry = repoContribs.get(name) ?? { commits: 0, issues: 0, pullRequests: 0, reviews: 0, isPrivate: stat.isPrivate };
  ...
  const candidateMap = new Map<string, { nameWithOwner: string; isPrivate: boolean; isFork: boolean }>();
  for (const r of viewer.repositories) {
    candidateMap.set(r.nameWithOwner, { nameWithOwner: r.nameWithOwner, isPrivate: r.isPrivate, isFork: r.isFork });
  }
  for (const [name, stat] of repoContribs.entries()) {
    if (!candidateMap.has(name)) {
      candidateMap.set(name, { nameWithOwner: name, isPrivate: stat.isPrivate, isFork: false });
    }
  }

  const locResults = await fetchRepoLOC(viewer.id, Array.from(candidateMap.values()), dateRange);
  ...
  const grandTotal = Object.values(mergedDaily).reduce((sum, count) => sum + count, 0);
...
async function main() {
  ...
  if (isIncremental && existsSync(filePath)) {
    const existing = JSON.parse(readFileSync(filePath, 'utf-8')) as GitHubStatsOutput;
...
main().catch((err) => {
  console.error('\n[FATAL ERROR]', err instanceof Error ? err.stack || err.message : err);
  process.exit(1);
});
