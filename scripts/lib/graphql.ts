...
export function getGitHubToken(): string {
  ...
  try {
    const cliToken = execFileSync('gh', ['auth', 'token'], { encoding: 'utf-8' }).trim();
    if (cliToken) return cliToken;
  } catch {}

  throw new Error('Missing GitHub token: Set GH_TOKEN or authenticate via "gh auth login"');
}
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (parsed.errors?.length) {
        throw new Error(`GraphQL errors: ${parsed.errors.map((e: { message: string }) => e.message).join('; ')}`);
      }
      ...
      const isTransient = /HTTP (499|502|503|403)|ETIMEDOUT|ECONNRESET/.test(message);
      ...
      const sanitizedMsg = token ? message.replaceAll(token, '[REDACTED]') : message;
